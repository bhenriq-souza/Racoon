...
dependency "dynamodb_table" {
  config_path = "../../../dynamodb_tables/users"
}

inputs = {
  role_name = "sign-up_lambda_role"
  ...
  policy_name = "sign-up_lambda_policy"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Effect   = "Allow",
        Resource = "${dependency.dynamodb_table.outputs.dynamodb_table_arn}"
      }
    ]
  })
}
